...
const Index = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/romanize-hindi-srt?model=claude-sonnet-4-6&chunk_size=100`, {
        method: "POST",
        body: formData,
      });
      if (!res.ok) {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4 py-12 overflow-hidden">
      ...
      >
        ...
        >
          ...
          <span className="text-xs font-body font-medium text-muted-foreground">Hindi SRT Romanizer</span>
        </motion.div>
        <h1 className="text-4xl font-display font-bold tracking-tight text-foreground sm:text-5xl">
          Romanize <span className="text-primary">Hindi SRT</span>
        </h1>
        <p className="mt-3 max-w-md text-base text-muted-foreground font-body">
          Upload a Hindi SRT file and generate a Romanized SRT using Anthropic.
        </p>
        {error && (
          ...
          {state === "processing" && (
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Anthropic is romanizing your Hindi subtitles</p>
              </div>
